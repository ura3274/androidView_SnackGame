...
class Mouth() {
    ...
    fun moveMouth(pos: Point, dir:String){
        direction = dir
        posX = pos.x.toFloat()
        posY = pos.y.toFloat()
        when(dir){
            "r", "l"->{
                pos1X = pos.x.toFloat()
                pos1Y = (pos.y+20).toFloat()
                pos2X = pos.x.toFloat()
                pos2Y = (pos.y-20).toFloat()

            }
            "d","u"->{
                pos1X = (pos.x-20).toFloat()
                pos1Y = pos.y.toFloat()
                pos2X = (pos.x+20).toFloat()
                pos2Y = pos.y.toFloat()
            }
        }
    }
    fun drawMouth(canvas: Canvas, paint: Paint){
        ...
        val rectBend = RectF(posX-20f, posY-20f, posX+20f, posY+20f)
        val path = Path()
